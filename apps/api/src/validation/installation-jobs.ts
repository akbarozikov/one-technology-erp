...
export interface InstallationJobCreateResultDraftInput {
...
export function parseInstallationJobCreateResultDraft(
  body: JsonObject,
  errors: Failures
): InstallationJobCreateResultDraftInput | null {
  const result_status = optionalEnum(
    body,
    "result_status",
  ...
  if (result_status === null || errors.length > 0) {
    return null;
  }
